...
def educationSection(education_data, course_data):
    ...
    st.dataframe(coursework,column_config= {
        ...
        hide_index=True, # hides index next to course information
    )
    st.write("---")
educationSection(info.education_data, info.course_data) #the astrix makes the text bold
...
#Activities: 
def acitivitiesSection(leadership_data, activity_data):
    ...
    with tab2: 
        ...
        for title, details in activity_data.items():
            ...
            for bullet in details: 
                expander.write(bullet)
    st.write("---")
acitivitiesSection(info.leadership_data, info.activity_data)
